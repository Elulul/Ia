...
treeRegressor = DecisionTreeRegressor(criterion = "mse",random_state = 0, max_depth=4)

pca = PCA(n_components=50)
xtr=pca.fit_transform(xtrain2)
xte=pca.fit_transform(xtest2)

treeRegressor.fit(xtr,ytrain2)

ypred2 = treeRegressor.predict(xte)
...
tree.export_graphviz(treeRegressor,out_file='tree.dot',feature_names=df3.columns)  
ypred = treeRegressor.predict(xtest)

mse_tr = mean_squared_error(ypred,ytest)
r2_tr = r2_score(ytest,ypred)

#♦compute p-value of non correlation with the cible value
#H0 : La colonne n'est pas correlée avec la valeur cible

colonneCible=df3[df3.columns[-1]]
for col in df3.columns.drop([df3.columns[0],df3.columns[-1]]):
...
mse_rdf_all_var = mean_squared_error(ytest2,ypred)
